...
db = dataset.connect('sqlite:///bmw.db')
conn = sqlite.connect('bmw.db')

# a directory for caching file's we've already downloaded
CACHE_DIR = os.path.join(os.path.dirname(__file__), 'cache_bmw')
...
def scrape_missed_connections():
    ...
    response = requests.get(BASE_URL + "search/cta?query=528%7C535&minAsk=10000&maxAsk=35000&autoMinYear=2011&autoMakeModel=bmw")
